jQuery(document).ready(function ($) {
    // Use event delegation on the body to ensure the click handler works even if the button is re-rendered (common in Gutenberg).
    $('body').on('click', '#publish, .editor-post-publish-button__button', function (e) {
        var isGutenberg = typeof wp !== 'undefined' && wp.data && wp.data.select('core/editor');

        var title, content, hasImage;

        if (isGutenberg) {
            var editor = wp.data.select('core/editor');
            title = editor.getEditedPostAttribute('title');
            content = editor.getEditedPostContent();
            hasImage = editor.getEditedPostAttribute('featured_media') > 0;
        } else { // Classic Editor
            title = $('#title').val();
            if (typeof tinymce !== 'undefined' && tinymce.get('content')) {
                content = tinymce.get('content').getContent();
            } else {
                content = $('#content').val();
            }
            // In the classic editor, a value of -1 means no image.
            hasImage = $('#_thumbnail_id').val() !== '-1';
        }

        // Clean up content for a more reliable empty check.
        // This removes HTML comments (like Gutenberg block comments) and trims whitespace.
        if (content) {
            content = content.replace(/<!--[\s\S]*?-->/g, '').trim();
            // Also check for the default empty content from the classic editor.
            if (content === '<p><br data-mce-bogus="1"></p>') {
                content = '';
            }
        }

        var errors = [];
        if (!title || title.trim() === '') {
            errors.push('・キャンペーン名（タイトル）は必須です。'); // Title is required
        }
        if (!content || content.trim() === '') {
            errors.push('・内容（メッセージ）は必須です。'); // Message/content is required
        }
        if (!hasImage) {
            errors.push('・アイキャッチ画像（イメージ）は必須です。'); // Featured image is required
        }

        if (errors.length > 0) {
            e.preventDefault(); // Stop the form from submitting
            alert('以下の必須項目を入力してください：\n\n' + errors.join('\n')); // Please enter the following required fields:

            // Re-enable the UI so the user can make corrections.
            if (isGutenberg) {
                // In Gutenberg, we need to unlock the editor's saving state.
                wp.data.dispatch('core/editor').unlockPostSaving('sbs-validation-lock');
                $('.editor-post-publish-button__button').removeClass('is-busy');
            } else {
                $('#publishing-action .spinner').hide();
                $('#publish').removeClass('button-primary-disabled');
            }
            return false;
        }
    });
});
